...
const ChatRoom = ({ room, onBack }) => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      {/* Avatar Action Bubble - Compact Design */}
      {showAvatarBubble && (
        <>
          {/* Overlay to close bubble when clicking outside */}
          <div 
            className="fixed inset-0 z-40"
            onClick={closeAvatarBubble}
          />
          
          <div 
            className="fixed z-50 transform -translate-x-1/2 -translate-y-full"
            style={{
              left: `${avatarBubblePosition.x}px`,
              top: `${avatarBubblePosition.y - 8}px`
            }}
          >
            {/* Arrow pointing down - smaller */}
            <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-3 border-r-3 border-t-4 border-transparent border-t-gray-900"></div>
            
            <div className="bg-gray-900/98 backdrop-blur-xl rounded-xl border border-white/30 shadow-2xl p-2 min-w-36 max-w-44">
              <div className="flex items-center space-x-2 mb-2">
                <div className="w-8 h-8 rounded-full overflow-hidden border border-purple-500/40">
                  {showAvatarBubble.avatar_url ? (
                    showAvatarBubble.avatar_url.startsWith('data:') ? (
                      <img 
                        src={showAvatarBubble.avatar_url} 
                        alt="Avatar" 
                        className="w-full h-full object-cover" 
                      />
                    ) : (
                      <div 
                        className="w-full h-full"
                        dangerouslySetInnerHTML={{ 
                          __html: defaultAvatars.find(a => a.id === showAvatarBubble.avatar_url)?.svg || '' 
                        }}
                      />
                    )
                  ) : (
                    <div className="w-full h-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center">
                      <span className="text-white text-xs font-semibold">
                        {showAvatarBubble.nickname?.charAt(0)?.toUpperCase()}
                      </span>
                    </div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-white font-medium text-sm truncate">{showAvatarBubble.nickname}</div>
                </div>
              </div>
              
              <div className="flex space-x-1">
                <button
                  onClick={() => openPrivateChatFromAvatar(showAvatarBubble)}
                  className="flex-1 bg-blue-600/90 hover:bg-blue-600 text-white px-2 py-1.5 rounded-lg text-xs transition-colors duration-200 flex items-center justify-center space-x-1"
                >
                ...
                <button
                  onClick={() => addToFavoritesFromAvatar(showAvatarBubble.id)}
                  className="flex-1 bg-purple-600/90 hover:bg-purple-600 text-white px-2 py-1.5 rounded-lg text-xs transition-colors duration-200 flex items-center justify-center space-x-1"
                >
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                  </svg>
